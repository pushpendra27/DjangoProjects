...
from service.models import Cars, car_service
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if car_service.objects.exists() or Cars.objects.exists():
            print('Car data already loaded...exiting.')
            print(ALREADY_LOADED_ERROR_MESSAGE)
            return
        print("Creating services data")
        for service_name in SERVICE_NAMES:
            vac = car_service(name=service_name)
            vac.save()
        print("Loading car data for cars available for service")
        for row in DictReader(open('/home/pushpendra/CarManagement/car_data.csv')):
            car = Cars()
